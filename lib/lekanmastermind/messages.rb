...
module Lekanmastermind
  class Messages
    def welcome_msg
      puts 'Welcome to MASTERMIND/n Would you like to (p)lay, read the
    ...
    def prompt_for_player1_name
      puts 'Enter your name'
    end

    def prompt_for_player2_name
    ...
    def congratulatory_message(player, trial, time)
      puts "Congratulation #{player.name}!! You guessed the correct sequence
      '#{player.guess}' in #{trial} trial(s) over #{time / 3600}hrs
    ...
    def guess_process(guess, exact, partial, chances)
